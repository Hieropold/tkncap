 ...
 * variable is absent or contains an unrecognised value, the level defaults to
 * INFO. Valid values are "debug", "info", "warn", "error" (case-insensitive).
 *
 ...
 * environment and maps it to a slog.Level. Falls back to slog.LevelInfo for
 * missing or unrecognised values. Uses a TextHandler writing to stderr so that
